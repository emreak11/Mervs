    ...
    for ti, tau in enumerate(taus):
        for ri, rho in enumerate(rhos):
            ...
            for rep in range(n_reps):
                ...
                theta = simulate_delayed_kuramoto_dynamic(
                    nbrs=nbrs, pos=pos, eps=eps, tau=float(tau), omega=omega,
                    T=T, dt=dt, noise_sigma=noise_sigma,
                    seed=seed0 + 100000*ti + 1000*ri + rep,
                    rewiring_rho=float(rho), rewiring_mode="global",
                    rewiring_interval=rewiring_interval,
                    local_candidates=local_candidates,
                )
                m, _, _ = defect_window_stats(theta, nbrs, threshold=threshold, tail_steps=tail_steps)
                rep_means_g.append(m)
            ...
            for rep in range(n_reps):
                ...
                theta = simulate_delayed_kuramoto_dynamic(
                    nbrs=nbrs, pos=pos, eps=eps, tau=float(tau), omega=omega,
                    T=T, dt=dt, noise_sigma=noise_sigma,
                    seed=seed0 + 200000*ti + 2000*ri + rep,
                    rewiring_rho=float(rho), rewiring_mode="local",
                    rewiring_interval=rewiring_interval,
                    local_candidates=local_candidates,
                )
                m, _, _ = defect_window_stats(theta, nbrs, threshold=threshold, tail_steps=tail_steps)
                rep_means_a.append(m)
            ...
            print(f"tau={tau:.3f}, rho={rho:.3f} -> alt={a:.4g}, global={g:.4g}, delta={a-g:.4g}", flush=True)

    return alt, glob, delt

def plot_heatmap(Z, taus, rhos, title, cmap="viridis"):
    plt.figure()
    im = plt.imshow(
        Z,
        aspect="auto",
...
if __name__ == "__main__":
    mean_period = 30.0
    taus = [0.0, 0.05*mean_period, 0.1*mean_period, 0.15*mean_period, 0.2*mean_period, 0.25*mean_period, 0.3*mean_period]
    rhos = [0.0, 0.02, 0.05, 0.1, 0.2]

    alt, glob, delt = run_phase_diagram(
        taus=taus,
        rhos=rhos,
        n_reps=5,     # önce küçük tut
        T=200.0,
        dt=0.05,
        tail_steps=200,
    )

    plot_heatmap(delt, taus, rhos, title="Phase diagram: Δdefect = alt_local - null_global")
    plot_heatmap(alt, taus, rhos, title="Phase diagram: defect (alt_local)")
