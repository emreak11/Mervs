...
# -------------------------
def defect_rate_proxy(theta_t, nbrs, threshold=DEFECT_THRESHOLD):
    """
...
def defect_window_stats(theta, nbrs, threshold=DEFECT_THRESHOLD, tail_steps=300):
    """
...
def run_phase_diagram(
    ...
    # Precompute positions / omega once (shared across runs)
    pos = build_grid_positions(nx, ny, dx=1.0, dy=1.0)
    N = nx * ny
    mean_omega = 2*np.pi / mean_period
    base_rng = np.random.default_rng(seed0)
    ...
    # Matrices: rows = taus, cols = rhos
    alt = np.zeros((len(taus), len(rhos)))
    glob = np.zeros((len(taus), len(rhos)))
    delt = np.zeros((len(taus), len(rhos)))
    ...
    for ti, tau in enumerate(taus):
        for ri, rho in enumerate(rhos):
            ...
            for rep in range(n_reps):
                seed_base = seed0 + 100000*ti + 1000*ri + rep

            # ortak başlangıç koşulu (theta0) üret
                rng0 = np.random.default_rng(seed_base)
                theta0 = rng0.uniform(0, 2*np.pi, size=N)

            # --- null_global ---
                nbrs_g = build_neighbor_graph_grid(nx, ny, mode="von_neumann")
                theta_g = simulate_delayed_kuramoto_dynamic(
                    nbrs=nbrs_g, pos=pos, eps=eps, tau=float(tau), omega=omega,
                    T=T, dt=dt, noise_sigma=noise_sigma,
                    seed=seed_base,
                    theta0=theta0,
                    rewiring_rho=float(rho),
                    rewiring_mode="global",
                    rewiring_interval=rewiring_interval,
                    local_candidates=local_candidates,
            )
                m_g, _, _ = defect_window_stats(
                    theta_g, nbrs_g, threshold=threshold, tail_steps=tail_steps
            )

            # --- alt_local ---
                nbrs_a = build_neighbor_graph_grid(nx, ny, mode="von_neumann")
                theta_a = simulate_delayed_kuramoto_dynamic(
                    nbrs=nbrs_a, pos=pos, eps=eps, tau=float(tau), omega=omega,
                    T=T, dt=dt, noise_sigma=noise_sigma,
                    seed=seed_base,
                    theta0=theta0,
                    rewiring_rho=float(rho),
                ...
                m_a, _, _ = defect_window_stats(
                    theta_a, nbrs_a, threshold=threshold, tail_steps=tail_steps
            )

                rep_means_g.append(m_g)
                rep_means_a.append(m_a)

            g = float(np.mean(rep_means_g))
            a = float(np.mean(rep_means_a))
        ...
        print(
            f"tau={tau:.3f}, rho={rho:.3f} -> alt={a:.4g}, global={g:.4g}, delta={a-g:.4g}",
            flush=True,
        )


    return alt, glob, delt

def plot_heatmap(Z, taus, rhos, title, cbar_label="value", fname=None, cmap="viridis"):
    plt.figure()
    im = plt.imshow(
        Z,
        aspect="auto",
        origin="lower",
        extent=[min(rhos), max(rhos), min(taus), max(taus)],
        interpolation="nearest",
        cmap=cmap,
    )
    plt.colorbar(im, label=cbar_label)
    plt.xlabel("rho (rewiring rate)")
    plt.ylabel("tau (delay)")
    plt.title(title)
    plt.tight_layout()
...
def summarize_delta(delt, area_thr=0.02, top_q=0.90):
    """
    delt: 2D array (taus x rhos), Δdefect = alt - global
    area_thr: |Δ| bunun üstündeyse 'meaningful' say
    top_q: üst quantile (0.90 -> top 10%)
    """
    absd = np.abs(delt)
    q = np.quantile(absd, top_q)
    top_mean = float(absd[absd >= q].mean()) if np.any(absd >= q) else 0.0
    area = float((absd >= area_thr).mean())
    mx = float(absd.max())
    return {
        "max_abs": mx,
        "top_q": top_q,
...
if __name__ == "__main__":
    ...
    # AŞAMA 3: zoom-in tarama
    taus = np.linspace(0.22 * P, 0.34 * P, 9)
    rhos = [0.0, 0.02, 0.04, 0.06, 0.08, 0.10, 0.12]

    for seed0 in (0, 1):
        print(f"\n=== ZOOM-IN | seed0 = {seed0} ===", flush=True)

        alt, glob, delt = run_phase_diagram(
            taus=taus,
            rhos=rhos,
            n_reps=5,
            T=200.0,
        ...
        ti, ri = np.unravel_index(np.argmax(np.abs(delt)), delt.shape)
        print(
            "MAX |Δdefect| at:",
            f"tau={taus[ti]:.3f}",
            f"rho={rhos[ri]:.3f}",
            f"value={delt[ti, ri]:.5f}",
            flush=True,
        )

        stats = summarize_delta(delt, area_thr=0.02, top_q=0.90)
        print(
            f"SUMMARY |Δ|: max={stats['max_abs']:.5f} | "
            f"Top10%mean={stats['top_mean']:.5f} | "
            f"Area(|Δ|>=0.02)={100*stats['area_frac']:.1f}%",
            flush=True,
        )

        plot_heatmap(
            delt, taus, rhos,
            title=f"Δdefect ZOOM (alt - global) | seed0={seed0}",
            cbar_label="Δdefect",
            fname=f"phase_delta_zoom_seed{seed0}.png",
        )
